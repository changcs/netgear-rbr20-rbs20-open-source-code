...
# package list
PKGS="xagent vzdaemon remGenie upagent"
...
do
	...
	/usr/bin/d2 -w XagentCtrl.xcenv
	
	case "$(/usr/bin/d2 -s XagentCtrl.xcenv)" in
	dev)
	...
	*)
	   # some sort of error
	   echo "Unsupported xcloud env: $(/usr/bin/d2 -s XagentCtrl.xcenv)" > /dev/kmsg 
	   ;;
	esac
	
	# reset value back
	/usr/bin/d2 -c XagentCtrl.xcenv none
