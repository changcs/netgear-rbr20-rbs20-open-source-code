#!/bin/sh
CONFIG=/bin/config
wl2g_bh_ap=`config get wlg_BACKHAUL_AP`
wl2g_bh_sta=`config get wlg_BACKHAUL_STA`
wl2g_gt_ap=`config get wlg_GUEST_AP`
wl2g_fh_ap=`config get wlg_NORMAL_AP`

project=`cat /tmp/orbi_project`
case $project in
    Orbipro|Desktop|OrbiOutdoor)
        wl5g_bh_ap=`config get wla_2nd_BACKHAUL_AP`
        wl5g_bh_sta=`config get wla_2nd_BACKHAUL_STA`
        wl5g_fh_ap=`config get wla_NORMAL_AP`
        wl5g_gt_ap=`config get wla_GUEST_AP`
        ;;
    Orbimini)
        wl5g_bh_ap=`config get wla_BACKHAUL_AP`
        wl5g_bh_sta=`config get wla_BACKHAUL_STA`
        wl5g_fh_ap=`config get wla_2nd_NORMAL_AP`
        wl5g_gt_ap=`config get wla_2nd_GUEST_AP`
        ;;
esac
...
#	wl5g_backhaul=`config get wl5g_BACKHAUL_STA`
	wl2g_backhaul_not_connected=`iwconfig $wl2g_bh_sta 2>/dev/null | grep "Not-Associated"`
	wl5g_backhaul_not_connected=`iwconfig $wl5g_bh_sta 2>/dev/null | grep "Not-Associated"`
