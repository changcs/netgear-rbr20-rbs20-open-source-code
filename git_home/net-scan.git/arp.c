...
InterfaceType_t get_ifstr(char *str)
{
	...
	else if(!strcmp(str,"3rd-5G"))return THIRD_5G_IF;
	...
	else if(!strcmp(str,"3rd-5G-GUEST"))return THIRD_5G_GUEST_IF;
#endif
#ifdef HAVE_GUEST_WIRELESS2
...
char tri_band_transform_flag[16] = "0";
int get_interface_info () {
	...
	if(!config_match("wla_2nd_operation_mode",tri_band_transform_flag))
	{
		strcpy(tri_band_transform_flag, config_get("wla_2nd_operation_mode"));
		system("/etc/init.d/netscan_if.init");
			...
			if (if_ele[i].port == portnum) {
				switch(if_ele[i].if_type) {
					...
					case THIRD_5G_IF:
					        ...
					        break;
#ifdef HAVE_GUEST_NETWORK
					case THIRD_5G_GUEST_IF:
					        u->ctype = THIRD_5G_GUEST;
					        u->atype = BASE_ATT;
					        break;
#endif
					case SATELLITE_IF:
